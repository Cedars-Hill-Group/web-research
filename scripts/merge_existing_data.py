...
from scripts.merge_markdown_db import append_markdown_to_company, choose_match  # noqa: E402
...
def run_merge(source_dir: Path, db_dir: Path, keep_merged_source: bool = False) -> int:
    ...
    for company_dir in sorted(source_dir.iterdir()):
        ...
        for md_file in md_files:
            if append_markdown_to_company(md_file, target_file, company_display=company_name):
...
def main() -> int:
    ...
    return run_merge(source_dir=source_dir, db_dir=db_dir, keep_merged_source=args.keep_merged_source)
